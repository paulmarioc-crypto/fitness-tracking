...
- **CrossTrainingEntry** — bike/soccer/volleyball/other: duration, intensity,
  HR, power, notes. Carries a `source` field (`manual` today; `google_fit` /
  `fitbit` reserved) so an external sync can be bolted on later — see
  `src/integrations/ActivityProvider.ts` for the interface that keeps
  ingestion separate from manual entry.
...
- **Train** — active session logging (tap-to-expand exercise, weight/reps/RIR
  entry, inline set editing) and bike/soccer/volleyball quick-log forms.
...
- **Cross-Training Analysis** (More → Cross-training analysis) — weekly
  minutes by activity, weekly gym volume, bike HR/power trend, soccer/
  volleyball intensity mix.
